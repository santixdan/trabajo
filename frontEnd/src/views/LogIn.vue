        ...
        <q-card style="margin: 0px;" class="my-card">
            <q-card-actions class="cardContent" align="center">
                <div class="column items-center q-mt-md">
                    <img style="height: 100px; width: 100px;" src="./../assets/images/Claro-rojo 1.svg">
                </div>
                <div class="q-pa-md" style="max-width: 300px">
                    <q-form @submit="authenticateUser()" class="q-gutter-md">
                        <q-input type="email" v-model="email" label="Correo" lazy-rules
                            :rules="[val => (val && val.length > 0) || 'Por favor, dígite el correo']" />
                        <q-input :type="isPwd ? 'password' : 'text'" v-model="password" label="Contraseña" lazy-rules
                            @paste.prevent
                        ...
                        <q-btn unelevated rounded no-caps label="Iniciar sesión" class="btn text-bold" color="primary"
                            type="submit" :loading="loading" />
                        <div>
                            <q-btn unelevated rounded no-caps label="Olvidé mi contraseña" color="grey-9" flat
                                class="q-ml-sm " @click="(modalForget = true)" />
                        </div>
                    </q-form>
                </div>
            </q-card-actions>
...
import { notifyErrorRequest, notifySuccessRequest } from "./../composables/notify.js"
